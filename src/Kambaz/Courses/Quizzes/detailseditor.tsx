...
import { setSelectedQuiz, updateQuiz, addQuiz, setQuestions } from "./reducer";
...
export default function QuizDetailsEditor() {
    ...
    const saveQuiz = async () => {
        try {
            let savedQuiz;
            if (isNewQuiz) {
                savedQuiz = await quizzesClient.createQuiz(courseId!, {
                    title: formState.title,
                    course: courseId
                });
                
                savedQuiz = await quizzesClient.updateQuiz(savedQuiz._id, {
                    ...formState,
                    course: courseId
                });
                
                dispatch(addQuiz(savedQuiz));
                navigate(`/Kambaz/Courses/${courseId}/Quizzes/${savedQuiz._id}`);
            } else {
                savedQuiz = await quizzesClient.updateQuiz(quizId!, {
                    ...formState,
                    course: courseId
                });
    ...
    const saveAndPublishQuiz = async () => {
        try {
            ...
            if (isNewQuiz) {
                ...
                await quizzesClient.publishQuiz(savedQuiz._id, true);
                dispatch(addQuiz({...savedQuiz, published: true}));
            } else {
                savedQuiz = await quizzesClient.updateQuiz(quizId!, {
                    ...formState,
                    course: courseId
                ...
                dispatch(updateQuiz({...savedQuiz, published: true}));
            }
    ...
    return (
        <div className="quiz-editor">
            <div className="d-flex justify-content-between align-items-center mb-4">
                <h2>{isNewQuiz ? "Create New Quiz" : `Edit Quiz: ${formState.title}`}</h2>
                <div>
